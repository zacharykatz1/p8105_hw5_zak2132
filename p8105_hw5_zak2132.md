...
study_df = tibble(
  ...
  separate(participant_id, into = c("arm", "study_id", sep = "_")) %>% 
  ...
  select(study_id, arm, everything()) %>% 
...
| study\_id | arm     | week | data\_point |
|:----------|:--------|-----:|------------:|
| 01        | Control |    1 |        0.20 |
| 01        | Control |    2 |       -1.31 |
| 01        | Control |    3 |        0.66 |
...
```
    ...
    ##  $ study_id  : chr [1:160] "01" "01" "01" "01" ...
...
variables, which include `study_id` (unique identifier for each
...
study_df %>% 
  ggplot(
    aes(
      ...
      group = interaction(arm, study_id),
...
<img src="p8105_hw5_zak2132_files/figure-gfm/unnamed-chunk-2-1.png" width="90%" />
